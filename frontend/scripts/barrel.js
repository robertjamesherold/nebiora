...
if (components.length === 0) {
  console.error(
    `No PascalCase .tsx or .ts files or subdirectories found in frontend/src/${relPath}`,
  );
  process.exit(1);
}
...
if (isCollection) {
  ...
  content =
    components.map((c) => `import ${c.name} from '${c.importPath}';`).join('\n\n') +
    '\n' +
    '\n' +
